...
def main(_):
    ...
    model_class = models.PytorchModel
    train_features = []
    dev_features = []
    part_label_features = []
    test_features_map = {}
    ...
    # train & eval
    model = SegmentModel.PytorchProxyModel(model_class,
                                           dim_info=dim_info,
    ...
    if FLAGS.do_train:
        ...
        for epoch in range(FLAGS.num_train_epochs):
            ...
            if len(part_label_features) == 0:
                _, _, _, total_loss, total_step = full_label_running(model, train_features, dim_info, config,
                                                                     batch_size=FLAGS.train_batch_size,
                                                                     is_training=True, show_info=True)
            else:
                if FLAGS.mix_pl_data:
                    total_loss, total_step = mix_training(model, train_features, part_label_features, dim_info, config,
                                                          batch_size=FLAGS.train_batch_size,
                                                          batch_base=not FLAGS.corpus_weighting, show_info=True)
                else:
                    total_pl_loss = 0
                    total_pl_step = 0
                    if epoch % FLAGS.whole_pl_training_epoch == 0:
                        total_pl_loss, total_pl_step = part_label_training(model, part_label_features, dim_info, config,
                                                                           batch_size=FLAGS.train_batch_size,
                                                                           show_info=True)
            ...
            if FLAGS.do_eval:
                dev_ground_true, dev_prediction, dev_texts, dev_loss, dev_step = full_label_running(
                    model, dev_features, dim_info, config, batch_size=FLAGS.eval_batch_size,
                    is_training=False)
                ...
                tf.logging.info("Epoch: %d Dev Dataset Best F1: %.5f" % (best_epoch, best_valid_f1))
            ...
            if FLAGS.debug_mode:
                for domain, test_features in test_features_map.items():
                    predict_ground_true, predict_prediction, predict_texts, predict_loss, predict_step = full_label_running(
                        model, test_features, dim_info, config,
...
def mix_training(model, features, pl_features, dim_info, config, batch_size, batch_base=False, show_info=False):
    total_loss = 0.
    step = 0
    start_time = datetime.now()
    if batch_base:
    ...
    for step, (input_ids, input_dicts, label_ids, seq_length) in enumerate(data_iterator):
        ...
        loss = model.train_step(features)
...
def part_label_training(model, features, dim_info, config, batch_size, show_info=False):
    total_loss = 0.
    step = 0
    start_time = datetime.now()
...
def full_label_running(model, features, dim_info, config, batch_size, is_training=False, show_info=False):
    total_loss = 0.
    step = 0
    ground_true = []
    predictions = []
    ...
    for step, (input_ids, input_dicts, label_ids, seq_length) in enumerate(
        ...
        if is_training:
            ...
            prediction = model.predict_step(features)
            loss = 0
            true_batch_size = len(input_ids)
            if dim_info.label_dim != 1:
                ground_true.extend(
                    [np.argmax(label_ids[i, :length[i]], axis=-1).tolist() for i in range(true_batch_size)])
            else:
